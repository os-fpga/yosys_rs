...
Fmt AstNode::processFormat(int stage, bool sformat_like, int default_base, size_t first_arg_at, bool may_fail) {
	...
	for (size_t index = first_arg_at; index < children.size(); index++) {
		...
		while (node_arg->simplify_original(true, stage, -1, false)) { }
...
void AstNode::annotateTypedEnums(AstNode *template_node)
{
	...
	if (template_node->attributes.count(ID::enum_type)) {
		...
		while (enum_node->simplify_original(true, 1, -1, false)) { }
...
bool AstNode::simplify(bool const_fold, int stage, int width_hint, bool sign_hint)
{
	     ...
	     str == "$write"   || str == "$writeb"   || str == "$writeh"   || str == "$writeo"))
	{
		if (!current_always) {
			...
			for (auto node : children)
				while (node->simplify_original(true, stage, -1, false)) {}
	...
	case AST_STRUCT_ITEM:
		if (is_custom_type) {
			...
			while (template_node->simplify_original(const_fold, stage, width_hint, sign_hint)) {};
...
bool AstNode::simplify_original(bool const_fold, int stage, int width_hint, bool sign_hint)
{
   maxNbSimplifyCalls = 0;

   bool did_something = simplify(const_fold, stage, width_hint, sign_hint);
...
bool AstNode::mem2reg_as_needed_pass2(pool<AstNode*> &mem2reg_set, AstNode *mod, AstNode *block, AstNode *&async_block)
{
			...
			children[0]->children[0]->children[0]->type != AST_CONSTANT)
	{
		...
		while (wire_addr->simplify_original(true, 1, -1, false)) { }
		...
		while (wire_data->simplify_original(true, 1, -1, false)) { }
	...
	if (type == AST_IDENTIFIER && variables.count(str)) {
		...
		if (!children.empty()) {
			...
			if (!children.at(0)->replace_variables(variables, fcall, must_succeed))
				return false;
			while (simplify_original(true, 1, -1, false)) { }
			if (!children.at(0)->range_valid) {
				if (!must_succeed)
					return false;
...
AstNode *AstNode::eval_const_function(AstNode *fcall, bool must_succeed)
{
	...
	while (!block->children.empty())
	{
		...
		{
			while (stmt->simplify_original(true, 1, -1, false)) { }
			if (!stmt->range_valid) {
				if (!must_succeed)
					goto finished;
		...
		if (stmt->type == AST_WHILE)
		{
			...
			while (cond->simplify_original(true, 1, -1, false)) { }
		...
		if (stmt->type == AST_REPEAT)
		{
			...
			while (num->simplify_original(true, 1, -1, false)) { }
		...
		if (stmt->type == AST_CASE)
		{
			...
			while (expr->simplify_original(true, 1, -1, false)) { }
